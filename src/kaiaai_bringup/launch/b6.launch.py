...
def make_nodes(context: LaunchContext, robot_model, map_path, use_sim_time, slam, lidar_model, urdf_path):
    ...
    return [
        ...
        # 3. RViz 单独设置延迟
        # 仅 RViz 需要等待 TF 稳定，将其包装在 TimerAction 中
        TimerAction(
            period=3.0, # 降低到 3 秒，仅用于 UI 渲染缓冲
            actions=[
                Node(
                    package='rviz2',
                    executable='rviz2',
...
def generate_launch_description():
    ...
    # ---------- 1. 基础硬件层 (立即启动) ----------
    # 必须最先启动 Agent，确保底层的 odom 话题能尽快被上位机接收
    ...
    # ---------- 2. 定位与状态层 (立即启动) ----------
    # EKF 节点需要尽早运行，开始监听来自 micro-ROS 的数据并发布 odom -> base_link
    ekf_node = Node(
    ...
    # ---------- 3. 核心功能层 (延迟缩减) ----------
    # 原本 7 秒的延迟太长，缩短至 2 秒左右。
    # 只要 EKF 输出了第一帧变换，Nav2 就会瞬间激活，不再超时。
    app_layer = TimerAction(
        period=2.0, 
        actions=[
            OpaqueFunction(
                function=make_nodes,
